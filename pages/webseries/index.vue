  ...
  <p style="cursor:pointer" @click="hidePopup()">
    ...
    <template v-if="seriesItem5">
      <div class="bg-class" v-if="seriesItem5.series_id==0">
        <img :src="seriesItem5.thumbs.original" class="image-header-all" />
  ...
  methods: {
    ...
    getMovies(seriesLimit) {
      try {
        ...
        this.$axios
          .get(`/videos/list/?categories=27&limit=${seriesLimit}`)
          .then(
            response => {
              ...
              //     "episodes"
              //   ];
              let tempdata = response.data.videos.category_videos["27"];
              let data = response.data.videos.category_videos["27"];
    ...
    getSeries(seriesLimit) {
      try {
        this.$axios.get(`/series/list/?categories=5&limit=${seriesLimit}`).then(
          response => {
            console.log("response 12 cat", response);
            // let tempdata =
            //   response.data.series.series_by_id["0"]["seasons"]["0"][
            //     "episodes"
            //   ];
            let tempdata = response.data.series.category_series["5"];
            let data = response.data.series.category_series["5"];
            console.log("this.limit", data);
            // for (var i = 0; i < 10; i++) {
            //   data.push(tempdata[i]);
            // }
            ...
            if (this.moviesData) {
              ...
              if (this.seriesItem5 == null) {
                this.seriesItem5 = this.seriesData[0];
              }
