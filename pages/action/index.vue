  ...
  methods: {
    ...
    getMovies(seriesLimit) {
      try {
        ...
        this.$axios
          .get(`/videos/list/?categories=12&limit=${seriesLimit}`)
          .then(
            response => {
              ...
              //     "episodes"
              //   ];
              let tempdata = response.data.videos.category_videos["12"];
              let data = response.data.videos.category_videos["12"];
    ...
    getSeries(seriesLimit) {
      try {
        this.$axios.get(`/series/list/?categories=9&limit=${seriesLimit}`).then(
          response => {
            console.log("response 12 cat", response);
            // let tempdata =
            //   response.data.series.series_by_id["0"]["seasons"]["0"][
            //     "episodes"
            //   ];
            let tempdata = response.data.series.category_series["9"];
            let data = response.data.series.category_series["9"];
            console.log("this.limit", data);
            // for (var i = 0; i < 10; i++) {
            //   data.push(tempdata[i]);
            // }
