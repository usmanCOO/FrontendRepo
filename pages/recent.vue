		...
		<div class="Vidlix_active_layout mt-5 pt-2">
			<div class="rowContainer_layout active">
				<v-flex xs1 class="pt-3 pl-3">
			  	<div pl-2 class="title">{{$t("recent")}}</div>
				</v-flex>
				<template v-if="recentVideos">
					<template v-for="(recentVids, index) in recentVideos">
						<template v-if="recentVids && recentVids.length > 0">
							<div v-if="recentVideos && recentVideos.length > 0" class="rowContainer_title_card"  id="row-1">
								<v-layout row wrap my-2 mx-0 py-3>
									<v-flex v-for="recentVid in recentVids" :key="recentVid.videoid" lg2 md4 sm4 xs6 class="item_thumb">
										<v-hover>
											<v-card slot-scope="{ hover }" dark class="block-holder mx-1 my-1" :ref="`conventional`+index+recentVid.videoid">
												<router-link class="sliderimg" to="">
													<v-img
													:src="hover ? recentVid.gif != null ? recentVid.gif : recentVid.thumbs['416x260'] : recentVid.thumbs['original']"
													:lazy-src="`${recentVid.thumbs['168x105']}`"
													aspect-ratio="1"
												...
												<div class="hover__detaillist">
													<router-link :to="`/watch/${recentVid.videoid}`">
														<div class="bob-overview-wrapper"> 
															<div class="title mx-2 mt-2">{{recentVid.title}}</div>
															<time class="caption time mx-2">{{recentVid.duration}}</time>
															<p class="caption dec mx-2">{{recentVid.description}}</p>
														</div>
													</router-link>
													<div class="btn__show align-center column  justify-start fill-height">
														<v-btn fab small @click="likeVideo(recentVid)">
															<v-icon dark>favorite</v-icon>
														</v-btn>
														<v-btn fab small @click="addToMyList(recentVid)">
															<v-icon dark>add</v-icon>
														</v-btn>
													</div>
													<div class="click_detail" v-on:click="getMeVideo(recentVid.videoid,`conventional`+index)">
														<v-icon dark>expand_more</v-icon>
													</div>
												</div>
												<span :ref="`conventional`+index+recentVid.videoid+'Arrow'"></span>
											</v-card>
										</v-hover>
									</v-flex>
								</v-layout>
	...
	methods:{
		...
		setPageData(){
			try{
				...
				this.$axios.get(`/videos/list/?recent=1&page=${this.page}&paginate=1&maturity_level=${this.$store.getters.currentProfile.maturity_level}`).then((response) => {
					this.dataLoading = false
					let baseObj = response.data.videos
					let dataArray = baseObj.recent.data
					this.currentPage = baseObj.recent.current_page
  				this.lastPage = baseObj.recent.last_page
