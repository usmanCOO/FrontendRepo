       ...
       <div class="bg-class" v-else>
        ...
        <div class="col-md-12 d-flex justify-content-center header-all">
          ...
          <div class="header-all-text">
            ...
            <br />
            <p style="width:37%">{{seriesItem5.series_description}}</p>
  ...
  methods: {
    ...
    getMovies(seriesLimit) {
      try {
        ...
        this.$axios
          .get(`/videos/list/?categories=17&limit=${seriesLimit}`)
          .then(
            response => {
              ...
              //     "episodes"
              //   ];
              let tempdata = response.data.videos.category_videos["17"];
              let data = response.data.videos.category_videos["17"];
    ...
    getSeries(seriesLimit) {
      try {
        this.$axios.get(`/series/list/?categories=4&limit=${seriesLimit}`).then(
          response => {
            console.log("response 12 cat", response);
            // let tempdata =
            //   response.data.series.series_by_id["0"]["seasons"]["0"][
            //     "episodes"
            //   ];
            let tempdata = response.data.series.category_series["4"];
            let data = response.data.series.category_series["4"];
            console.log("this.limit", data);
            // for (var i = 0; i < 10; i++) {
            //   data.push(tempdata[i]);
            // }
