...
<template>
  <section class="home_editerpicker">
    ...
    >
      <div v-if="seriesDetails">
        <teaserPlayer
          ...
          :isPlay="isPlay"
        />
        <div v-if="seriesDetails.series_id!=0" class="header-all-text">
        ...
        <div v-if="seriesDetails.series_id==0" class="header-all-text">
          <h1 style="text-transform:capitalize">{{seriesDetails.title.toLowerCase().trim()}}</h1>
          <small
            v-for="(cat, index) in seriesDetails.categories"
            :key="index"
            style="padding-bottom:15px;"
          ...
          <div class="row col button-group">
            <v-btn class="btn btn-grey btn-daikho" @click="takeMeToWatchVideo(seriesDetails)">
              <v-icon>play_arrow</v-icon>&nbsp;Watch Now
            </v-btn>&nbsp;
            &nbsp;
      ...
      <div class="swiper-wrapper" @mouseover="showBody">
        <div v-for="(editorPick,index) in editorPicks" class="swiper-slide tuhumbinner">
          <img
          v-if="editorPick.series_id!=0" 
   ...
   <img
          v-if="editorPick.series_id==0" 
            :class="`imageSection grey lighten-2 imageSection_${index}`"
            :src="editorPick.thumbs['original']"
            aspect-ratio="1"
            :id="`imageSection_${index}`"
            :key="`imageSection_${index}`"
          />
...
export default {
  name: "editorPicks",
  props: ["editorPicks", "swiperOption"],
  data() {
    return {
      seriesDetails: null,
      activeIndex: 0,
  ...
  mounted() {
    ...
    try {
      console.log("editor", this.editorPicks
      );
      this.seriesDetails = this.editorPicks[0];
    ...
    try {
      this.swiperTop.on("slideNextTransitionStart", function() {
        ...
        self.seriesDetails = self.editorPicks[active_index];
        self.logData("slider_next", self.editorPicks[active_index]);
        setTimeout(function() {
          $(".videoSection").fadeOut("fast");
        }, 2500);
      });
    ...
    try {
      this.swiperTop.on("slidePrevTransitionStart", function() {
        ...
        self.activeIndex = active_index;
        self.seriesDetails = self.editorPicks[active_index];
        self.logData("slider_prev", self.editorPicks[active_index]);
