<template>
		<v-layout row wrap mt-5 mb-4> 
			<v-flex lg9 md9 sm12 xs12 mt-4>
				<!-- catagory_Move -->
				<div class="vd__catlist d-block mt-2">
					<div class="font-weight-black text-capitalize ml-3 headline d-inline">
						 {{$t('movies')}} 
					</div>
					<template v-if="videoCategories && videoCategories.length > 0"> 
						<v-menu offset-y  class="ml-3">
							...
							<v-list class="vdcategory">
								...
								<v-list-tile v-for="vCat in videoCategories" :to="'/movies/category/'+vCat.category_id" :key="vCat.category_id">
									<v-list-tile-title> 
										{{vCat.category_name}}
									</v-list-tile-title>
								</v-list-tile>
...
<script>
	export default{
		...
		methods:{
			setVideoCategories(){
				try {
	        this.$axios.get(`/videos/categories/`).then((response) => {
